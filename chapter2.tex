...
% 陈老视频14
\begin{lemma}\label{lemma:infty-relation}
...
\begin{example}\label{example-chapter2-3}
    讨论极限
    \begin{equation*}
...
\begin{theorem}[Stolz定理]\label{theorem:Stolz}
...
\end{example}
\begin{proof}
    取$x_n = a_1 + a_2 + \cdots + a_n$, $y_n = n$, 则有$\collect{y_n}$严格单调增加, 且$\mylim{n}{y_n} = +\infty$, 并且有:
    \begin{equation*}
        \mylim{n}{\frac{x_n-x_{n-1}}{y_n - y_{n-1}}} = \mylim{n}{a_n} = 0
    \end{equation*}

    则根据Stolz定理(定理~\ref{theorem:Stolz})得:
    \begin{equation*}
        \mylim{n}{\frac{x_n}{y_n}} = \mylim{n}{\mean{a}{n}} = a
    \end{equation*}
\end{proof}
...
\begin{example}
    求:
    \begin{equation*}
        \mylim{n}{\frac{1^k+2^k+\cdots+n^k}{n^{k+1}}}
    \end{equation*}
\end{example}
\begin{solution}
    取$x_n = 1^k + 2^k + \cdots + n^k$, $y_n = n^{k+1}$, 其中$y_n$严格单调增加, 且$\mylim{n}{y_n} = +\infty$, 将$(n-1)^{k+1}$进行多项式展开有:
    \begin{equation*}
        \mylim{n}{\frac{x_n-x_{n-1}}{y_n-y_{n-1}}} = \mylim{n}{\frac{n^k}{n^{k+1}- (n-1)^{k+1}}} = \mylim{n}{\frac{n^k}{(k+1)n^k - C^2_{n+1}n^{k-1} + \cdots + C^{n+1}_{n+1}(-1)^{n+1}}}
    \end{equation*}

    根据例题~\ref{example-chapter2-3}有:
    \begin{equation*}
        \mylim{n}{\frac{x_n-x_{n-1}}{y_n-y_{n-1}}} = \frac{1}{k+1}
    \end{equation*}

    则根据Stolz定理(定理~\ref{theorem:Stolz})得:
    \begin{equation*}
        \mylim{n}{\frac{x_n}{y_n}} = \mylim{n}{\frac{1^k+2^k+\cdots+n^k}{n^{k+1}}} = \frac{1}{k+1}
    \end{equation*}
\end{solution}
...
\begin{example}
    设$\mylim{n}{a_n} = a$, 求:
    \begin{equation*}
        \mylim{n}{\frac{a_1+2a_2+3a_3+\cdots+na_n}{n^2}}
    \end{equation*}  
\end{example}
\begin{solution}
    取$x_n = a_1+2a_2+3a_3+\cdots+na_n$, $y_n = n^2$, 其中$y_n$严格单调增加, 且$\mylim{n}{y_n} = +\infty$, 并且有:
    \begin{equation*}
        \mylim{n}{\frac{na_n}{n^2-(n-1)^2}} = \mylim{n}{\frac{na_n}{2n-1}} = \mylim{n}{\frac{a_n}{2-1/n}}
    \end{equation*}
    
    根据极限的四则运算有:
    \begin{equation*}
        \mylim{n}{\frac{na_n}{n^2-(n-1)^2}} = \frac{a}{2}
    \end{equation*}

    则根据Stolz定理(定理~\ref{theorem:Stolz})得:
    \begin{equation*}
        \mylim{n}{\frac{x_n}{y_n}} = \mylim{n}{\frac{a_1+2a_2+3a_3+\cdots+na_n}{n^2}} = \frac{a}{2}
    \end{equation*}
\end{solution}
到此为止, 我们已经熟悉了极限的求解方法, 因此接下来我们不会再将极限的求解步骤写得如此详细。

...
% 陈老视频14
\begin{lemma}\label{lemma:infity-relation}
...
\begin{proof}
    ...
    \end{equation*}
    根据引理~\ref{lemma:infity-relation}得$\collect{\frac{n}{\sin(n)}}$是无穷大量。

    因为$n \ge 1$, 因此有$\left|\arctan(n)\right| \ge \frac{\pi}{4}$, 根据引理~\ref{lemma:infity-relation}得$\collect{n \cdot \arctan(n)}$是无穷大量。
\end{proof}
